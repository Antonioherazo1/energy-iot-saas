...
from app.services.dashboard_service import get_accessible_organization_ids, get_billing_current_daily, get_billing_daily_per_channel, get_billing_monthly_energy, get_channel_day_series, get_device_status, get_energy_by_period, get_energy_slope, get_hourly_energy, get_latest_telemetry, get_realtime_currents, get_summary, recalculate_daily_energy
...
@router.get("/energy/slope", response_model=list[EnergySlopeRead])
def energy_slope(
    organization_id: uuid.UUID | None = None,
    limit: int = Query(default=30, ge=1, le=365),
...
def energy_hourly(
    ...
    return get_hourly_energy(db=db, user=current_user, date=date, device_id=device_id, organization_id=organization_id, bucket_seconds=bucket_seconds)
