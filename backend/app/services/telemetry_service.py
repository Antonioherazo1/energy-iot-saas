...
def store_raw_telemetry(db: Session, device_code: str, payload: TelemetryIn) -> RawTelemetry | None:
    ...
    if recorded_at.tzinfo is None:
        recorded_at = recorded_at.replace(tzinfo=timezone(timedelta(hours=payload.tz_offset or 0)))
    ...
    raw = RawTelemetry(
        ...
        ch1=float(payload.ch1) if payload.ch1 is not None else None,
        ch2=float(payload.ch2) if payload.ch2 is not None else None,
        ch3=float(payload.ch3) if payload.ch3 is not None else None,
        ch4=float(payload.ch4) if payload.ch4 is not None else None,
