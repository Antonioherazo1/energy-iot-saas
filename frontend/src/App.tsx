...
function FirmwarePanel({ token, onClose }: { token: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <section className="mx-4 max-h-[80vh] w-full max-w-md overflow-y-auto rounded-lg border border-line bg-white p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="mt-4 w-full h-11 rounded-md border border-line bg-white text-sm font-medium" onClick={onClose} type="button">Cerrar</button>
      </section>
    </div>
  );
}
