...
export function getHourlyEnergy(token: string, date: string): Promise<HourlyEnergy[]> {
  return request<HourlyEnergy[]>(`/dashboard/energy/hourly?date=${date}`, { token });
}
...
export function getBillingDailyChannels(token: string, deviceId: string): Promise<{ channel_number: number; channel_name: string; energy_kwh: string }[]> {
...
export function downloadTelemetryExcel(token: string, start?: string, end?: string, limit = 500): void {
